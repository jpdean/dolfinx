...
xdmf_write::compute_nonlocal_entities(const mesh::Mesh& mesh, int cell_dim)
{
  ...
  const int tdim = mesh.topology().dim();
  bool ghosted = (topology.size(tdim) > topology.size_local(tdim));
  if (!ghosted)
  {
  ...
  else
  {
    ...
    const std::int32_t ghost_offset_c = topology.size_local(tdim);
    const std::int32_t ghost_offset_e = topology.size_local(cell_dim);
                              ...
                              const std::string component)
{
  ...
  const std::size_t n_cells = mesh.topology().size_local(tdim);
